      ...
      <main className="mx-auto max-w-xl px-4 py-10">
        <header className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">
            Onoma<span className="text-lime-400">.</span>
          </h1>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="bpm" className="mb-1.5 block text-sm text-zinc-400">
                BPM <span className="text-zinc-600">(optional)</span>
              </label>
              <input
                id="bpm"
                type="number"
            ...
            <div>
              <label htmlFor="artist" className="mb-1.5 block text-sm text-zinc-400">
                Sounds like <span className="text-zinc-600">(optional)</span>
              </label>
              <input
                id="artist"
          ...
          <div>
            <label htmlFor="vibe" className="mb-1.5 block text-sm text-zinc-400">
              Vibe notes <span className="text-zinc-600">(optional)</span>
            </label>
            <textarea
              id="vibe"
              rows="2"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-xl bg-lime-400 py-3 font-semibold text-zinc-950 hover:bg-lime-300 disabled:cursor-wait disabled:opacity-60"
          >
            {loading ? 'naming…' : names ? 'Regenerate' : 'Generate names'}
          </button>
        </form>

        {error && (
          <p role="alert" className="mt-4 rounded-xl border border-red-900 bg-red-950/50 px-4 py-3 text-sm text-red-300">
            {error}
          </p>
        )}

        {names && !error && (
          <section className="mt-6 space-y-2" aria-label="generated names">
            {names.map((name) => (
